		...
		.then((response) => {
			...
			let anyCar =
				response[0].data.routes.length > 0
					? {
							...
							time: Math.floor(response[0].data.routes[0].legs[0].duration.value / 60),
							carbon: 0,
							saved: false,
							tag: 'anyCar'
						}
			...
			let walking =
				response[1].data.routes.length > 0
					? {
							...
							time: Math.floor(response[1].data.routes[0].legs[0].duration.value / 60),
							carbon: 0,
							saved: false,
							tag: 'walking'
						}
			...
			let transitRail =
				response[2].data.routes.length > 0
					? {
							...
							time: Math.floor(response[2].data.routes[0].legs[0].duration.value / 60),
							carbon: 0,
							saved: false,
							tag: 'transitRail'
						}
			...
			let bicycle =
				response[3].data.routes.length > 0
					? {
							mode: 'Bicycle',
							distance: Math.floor(response[3].data.routes[0].legs[0].distance.value / 1000),
							time: Math.floor(response[3].data.routes[0].legs[0].duration.value / 60),
							carbon: 0,
							saved: false,
							tag: 'bicycle'
						}
...
router.post('/saved-trip', isLoggedIn, (req, res) => {
	const { departure, arrival, transport, duration, carbon, distance, returnTrip, number, period } = req.body;
	const newTrip = {
		departure,
		arrival,
		transport,
...
router.delete('/deleteTrip/:_id', (req, res) => {
	Trip.findByIdAndRemove(req.params._id).then((dbRes) => res.json(dbRes)).catch((err) => console.log(err));
});
