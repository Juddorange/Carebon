...
export default function Home() {
	function timeConvert(n) {
		...
		if (rday > 1 && rhour > 0) return rday + ' days ' + rhour + ' h ' + rminutes + ' min ';
		if (rday > 1 && rhour === 0) return rday + ' days ' + rminutes + ' min ';
		if (rhours > 24 && rhour > 0) return rday + ' day ' + rhour + ' h ' + rminutes + ' min ';
					...
					if (values[0].err) {
						...
						for (let i = 0; i < searchedTrips.length; i++) {
							for (let j = 0; j < savedTrips.length; j++)
								if (
									searchedTrips[i].mode.toUpperCase() === savedTrips[j].transport &&
						...
						setTrip({ ...trip, errorMsg: '', transports: searchedTrips, mapSearch: false });
								...
								transport.error || (
									<div className="answer" key={i}>
										<ul onClick={handleResultOnclick}>
											<li className="iconLi">
												<i className={displayMode(transport.mode)} id={transport.mode} />
											</li>
											<li className="textLi">
												{trip.return === true ? transport.distance * 2 : transport.distance} km
											</li>
											<li className="textLi">
												{trip.return === true ? (
											...
											<li className="textLi">
												{trip.return === true ? transport.carbon * 2 : transport.carbon} kg
											</li>
											<li className="btnLi">
												<button className="saveTrip" onClick={() => handlesaveTrip(i)}>
													{transport.saved ? (
