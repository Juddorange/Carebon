...
export default function Profile(props) {
	...
	useEffect(() => {
		...
		api
			...
			.then((res) => {
				...
				let carbAvg = Math.floor(res.reduce((acc, cv) => acc + cv, 0) * 100 / arrLength) / 100;
	...
	function updateAvgProfile(value) {
		api.updateProfile('carbonAvg', value).then((res) => console.log(res)).catch((err) => console.log(err));
	}
	...
	function formatStats(arr) {
		...
		let averageNumber =
			Math.round(carbonEmittedPerTrip.reduce((acc, cv) => acc + cv, 0) / carbonEmittedPerTrip.length * 100) / 100;
		let toolTipLabels = [];
		let lineLabels = [];
		let average = [];

		for (let i = 0; i < arr.length; i++) {
			toolTipLabels.push(arr[i].departure + '-' + arr[i].arrival + ' (' + arr[i].transport + ')');
			lineLabels.push(i + 1);
		...
		let averageCarbonPerMonth = Math.round(100 * totalCarbonPerMonthArray.reduce((acc, cv) => acc + cv, 0)) / 100;
		...
		<div className="profile">
			...
			<div className="trip-charts">
				<div className="charts">
					<div className="line">
						<h2>Average carbon emission per month {statistics.averageCarbonPerMonth} kgs</h2>
						<GraphCarbonOverTime
